...
if (!class_exists("nxs_class_SNAP_IG")) { class nxs_class_SNAP_IG {
    ...
    function doPostToNT($options, $message){ $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>''); if (!class_exists("nxsAPI_IG")){ $badOut['Error'] .= "Instagram API not found"; return $badOut; } 
      ...
      $nt = new nxsAPI_IG(); $nt->opNm = $opNm;  if(!empty($options['ck'])) $nt->ck = $options['ck']; 
      if (!empty($message['session']) || !empty($options['session'])) { $sid = !empty($message['session'])?$message['session']:$options['session']; if (empty($nt->ck)) $nt->ck = array(); foreach ($ck as $ci=>$cc) if ( $ck[$ci]->name=='sessionid') unset($ck[$ci]);
         $c = new NXS_Http_Cookie( array('name' => 'sessionid', 'value' => $sid) ); $nt->ck[] = $c; 
      }
