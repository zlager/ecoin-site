<?php
/*
	Plugin Name: Quick Adsense Mobile
	Plugin URI: https://pluginmarket.net/
	Description: Quick Adsense Mobile offers a quicker & flexible way to insert Google Adsense or any Ads code into a blog post.
	Author: Quick Adsense Mobile
	Version: 2.0
	Author URI: https://codifica.me/
*/
...
$QData['Name'] = 'Quick Adsense Mobile';
$QData['Version'] = '2.0';
$QData['URI'] = 'https://pluginmarket.net/';
...
$QData['Default'] = array(
	...
	'qam_mobile1'=>false, 'qam_mobile2'=>false, 'qam_mobile3'=>false, 'qam_mobile4'=>false, 'qam_mobile5'=>false, 'qam_mobile60'=>false, 'qam_mobile61'=>false, 'qam_mobile62'=>false, 'qam_mobile7'=>false,
	'qam_pcc1'=>false, 'qam_pcc2'=>false, 'qam_pcc3'=>false, 'qam_pcc4'=>false, 'qam_pcc5'=>false, 'qam_pcc60'=>false, 'qam_pcc61'=>false, 'qam_pcc62'=>false, 'qam_pcc7'=>false
);
...
function qam_ads_admin_page_inc() {
	include('quick-adsense-mobile-admin.php');
}
function qam_ads_admin_page() {
	add_options_page("Quick Adsense mobile Options", "Quick Adsense mobile", "edit_pages", basename(__FILE__), "qam_ads_admin_page_inc");
}
function register_ads_settings() {
	...
	add_action('admin_menu', 'qam_ads_admin_page');
	add_action('admin_init', 'register_ads_settings');



function qam_ads_plugin_links($links,$file) {
	if($file==plugin_basename(__FILE__)) {
		array_unshift($links,'<a href="options-general.php?page='.basename(__FILE__).'">'.__('Setting').'</a>');
	}
	return $links;
}
add_filter('plugin_action_links','qam_ads_plugin_links',10,2);


function qam_plugin_activated() {
	global $QData;
...
register_activation_hook( __FILE__, 'qam_plugin_activated' );

global $versionwp;
$wpvcomp = (bool)(version_compare($versionwp, '3.1', '>='));
...
function qam_process_content($content)
{
	global $QData;
	global $ShownAds;
	global $AdsId;
	...
	if(	(is_feed()) ||
		...
		$content = qam_clean_tags($content); return $content;
	}
	...
	if( $ShownAds >= $AdsToShow ) { $content = qam_clean_tags($content); return $content; };
	...
	if( !count($AdsId) ) { $content = qam_clean_tags($content); return $content; };
	...
	if( !$offdef ) {
		...
		if( $midd1 && strpos($content,'<!--OffMiddle-->')===false) {  //2
		 if ((wp_is_mobile() && $mobiile2) || (!wp_is_mobile() && $pcc2) ){
		...
		if( $more1 && strpos($content,'<!--OffAfMore-->')===false) {
		 if ((wp_is_mobile() && $mobiile4) || (!wp_is_mobile() && $pcc4) ){  // 4
			$mmr = '<!--'.$r1.'-->';
			$postid = get_the_ID();
			$content = str_replace('<span id="more-'.$postid.'"></span>', $mmr, $content);
		 }
		}
		if( $begn1 && strpos($content,'<!--OffBegin-->')===false) {   // 1
			if ((wp_is_mobile() && $mobiile1) || (!wp_is_mobile() && $pcc1) ){
			    $content = '<!--'.$b1.'-->'.$content;
		  }
		}
		if( $endi1 && strpos($content,'<!--OffEnd-->')===false) {   //3
			if ((wp_is_mobile() && $mobiile3) || (!wp_is_mobile() && $pcc3) ){
			   $content = $content.'<!--'.$b2.'-->';
			}
		}
		if( $lapa1 && strpos($content,'<!--OffBfLastPara-->')===false){   // 5
			if ((wp_is_mobile() && $mobiile1) || (!wp_is_mobile() && $pcc5) ){
			   $sch = "<p>";
			   $content = str_replace("<P>", $sch, $content);
			   $arr = explode($sch, $content);
			   if ( count($arr) > 2 ) {
		...
		for($i=$rc, $z=0 ;$i>=1;$i--, $z++) {
			if ( $para1[$i] ){   // 6
	  		if ((wp_is_mobile() && $mobiile[$z]) || (!wp_is_mobile() && $pcc6[$z]) ){
		  		$sch = "</p>";
	  			$content = str_replace("</P>", $sch, $content);
		  		$arr = explode($sch, $content);
		...
		if ( $imge1 ){
		 if ((wp_is_mobile() && $mobiile7) || (!wp_is_mobile() && $pcc7) ){ // 9
	...
	$content = qam_clean_tags($content, true);
		...
		for( $i=1; $i<=count($AdsIdCus); $i++ ) {
			if( $showall || !$ismany || $beginend != $i ) {
				if( strpos($content,'<!--'.$cusads.$AdsIdCus[$i-1].'-->')!==false && in_array($AdsIdCus[$i-1], $AdsId)) {
					$content = qam_replace_ads( $content, $cusads.$AdsIdCus[$i-1], $AdsIdCus[$i-1] ); $AdsId = qam_del_element($AdsId, array_search($AdsIdCus[$i-1], $AdsId)) ;
					$ShownAds += 1; if( $ShownAds >= $AdsToShow || !count($AdsId) ){ $content = qam_clean_tags($content); return $content; };
	...
	if( $showall || !$ismany ) {
		...
		for( $i=1; $i<=$tcn; $i++ ) {
			if( strpos($content, '<!--Ads'.$AdsId[$tt].'-->')!==false ) {
				$content = qam_replace_ads( $content, 'Ads'.$AdsId[$tt], $AdsId[$tt] ); $AdsId = qam_del_element($AdsId, $tt) ;
				$ShownAds += 1; if( $ShownAds >= $AdsToShow || !count($AdsId) ){ $content = qam_clean_tags($content); return $content; };
			} else {
				$tt += 1;
			}
	...
	if( strpos($content, '<!--'.$cusrnd.'-->')!==false && ($showall || !$ismany) ) {
		...
		shuffle($AdsId);
		for( $i=1; $i<=$tcy; $i++ ) {
			$content = qam_replace_ads( $content, $cusrnd, $AdsId[0] ); $AdsId = qam_del_element($AdsId, 0) ;
			$ShownAds += 1; if( $ShownAds >= $AdsToShow || !count($AdsId) ){ $content = qam_clean_tags($content); return $content; };
		}
	}
	...
	if( strpos($content, '<!--RndAds-->')!==false && ($showall || !$ismany) ) {
		...
		for( $i=1; $i<=$tcy; $i++ ) {
			$tmp = $AdsIdTmp[0];
			$content = qam_replace_ads( $content, 'RndAds', $AdsIdTmp[0] ); $AdsIdTmp = qam_del_element($AdsIdTmp, 0) ;
			if($tmp != -1){$ShownAds += 1;}; if( $ShownAds >= $AdsToShow || !count($AdsIdTmp) ){ $content = qam_clean_tags($content); return $content; };
		}
	}
...
function qam_clean_tags($content, $trimonly = false) {
	global $QData;
	global $ShownAds;
	global $AdsId;
	global $beginend;
...
function qam_replace_ads($content, $nme, $adn) {
...
function qam_del_element($array, $idx) {
  $copy = array();
...
add_filter('the_content', 'qam_process_content');


function qam_ads_widget_register() {
	global $QData;
...
add_action('init', 'qam_ads_widget_register');
