...
if (!class_exists('Magazine_Prime_About')) {
	...
	class Magazine_Prime_About {
		 ...
		 * Single instance.
		 *
		 * @var Magazine_Prime_About $instance Instance object.
		 */
		...
		public static function init($config) {
			if (!isset(self::$instance) && !(self::$instance instanceof Magazine_Prime_About)) {
				self::$instance = new Magazine_Prime_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset($this->config['menu_name'])?$this->config['menu_name']:sprintf(esc_html__('About %s', 'magazine-prime'), $this->theme_name);
			$this->page_name     = isset($this->config['page_name'])?$this->config['page_name']:sprintf(esc_html__('About %s', 'magazine-prime'), $this->theme_name);
			$this->tabs          = isset($this->config['tabs'])?$this->config['tabs']:array();
			$this->page_slug     = $this->theme_slug.'-about';
			$this->page_url      = admin_url('themes.php?page='.$this->page_slug);
			$this->notice        = '<p>'.sprintf(esc_html__('Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'magazine-prime'), esc_html($this->theme_name)).'</p><p><a href="'.esc_url($this->page_url).'" class="button button-primary">'.sprintf(esc_html__('Get started with %1$s', 'magazine-prime'), $this->theme_name).'</a><a href="#" class="btn-dismiss" data-hello="world" data-userid="'.esc_attr(get_current_user_id()).'" data-nonce="'.esc_attr(wp_create_nonce('magazine_prime_dismiss_nonce')).'">'.esc_html__('Dismiss this notice', 'magazine-prime').'</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action('wp_ajax_magazine_prime_dismiss', array($this, 'dismiss_callback'));
			add_action('wp_ajax_nopriv_magazine_prime_dismiss', array($this, 'dismiss_callback'));
		}
		...
		public function render_page() {
			...
			?>&nbsp;
												-&nbsp;
			<?php echo esc_html($this->theme_version);
			?></h1>
		...
		public function render_current_tab_content() {
			...
			if (method_exists($this, $method)) {
				...
				printf(esc_html__('%s() method does not exist.', 'magazine-prime'), $method);
			}
		}
		...
		private function render_grid_item($item) {
			...
			?>
																	<a href="<?php echo esc_url($item['button_url']);?>" class="<?php echo esc_attr($button_class);?>" target="<?php echo esc_attr($button_target);?>"><?php echo esc_html($item['button_text']);
			?></a>
			<?php endif;?>
			</div><!-- .col -->
			<?php
		...
		public function useful_plugins() {
			...
			<?php if (true !== $tgmpa_complete):?>
																	<a href="<?php echo esc_url($tgmpa_url);?>" class="button button-primary"><?php esc_html_e('Manage Plugins', 'magazine-prime');
			?></a>
			<?php endif;?>
		...
		public function upgrade_to_pro() {
			...
			?>
																		<a href="<?php echo esc_url($content['button_url']);?>" class="<?php echo esc_attr($button_class);?>" target="<?php echo esc_attr($button_target);?>"><?php echo esc_html($content['button_text']);
			?></a>
			<?php endif;?>
		...
		public function assets($hook) {
			...
			if (in_array($hook, array('themes.php', 'appearance_page_'.$this->page_slug), true)) {
				wp_enqueue_style('magazine-prime-about', get_template_directory_uri().'/inc/about/css/about'.$min.'.css', array(), '1.0.0');
				wp_enqueue_script('magazine-prime-about', get_template_directory_uri().'/inc/about/js/about'.$min.'.js', array(), '1.0.0');
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta($user_id, 'magazine_prime_dismiss_status', true);
			?>
		...
		public function dismiss_callback() {
			...
			if (false === wp_verify_nonce($wpnonce, 'magazine_prime_dismiss_nonce')) {
				wp_send_json($output);
			}

			update_user_meta($userid, 'magazine_prime_dismiss_status', 1);
